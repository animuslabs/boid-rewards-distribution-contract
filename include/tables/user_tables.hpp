...
namespace gamerewards {
    ...
    struct [[eosio::table("userstat"), eosio::contract(CONTRACT_NAME)]] user_stat {
        name boid_id;
    ...
    typedef eosio::multi_index<"userstat"_n, user_stat> user_stat_table;

    // Table for user game history
    struct [[eosio::table("gamehistory"), eosio::contract(CONTRACT_NAME)]] game_history {
        uint64_t history_id;
        name boid_id;
        uint64_t record_id;
        uint64_t score;
